...
def plot_telemetry(telemetry: List[TelemetryFrame], save_path: Optional[str] = None):
    ...
    fig.suptitle('Project BIOS — Agent Performance Analysis', fontsize=18, fontweight='bold')

    # 1. Vital Signs
    axs[0, 0].plot(ticks, [t.energy for t in telemetry], label='Energy', color='#2ca02c', linewidth=2)
    axs[0, 0].plot(ticks, [t.integrity for t in telemetry], label='Integrity', color='#1f77b4', linewidth=2)
    axs[0, 0].set_title('Energy & Integrity', fontweight='bold')
    axs[0, 0].legend()
    axs[0, 0].grid(True, alpha=0.3)

    # 2. Emotional State
    axs[0, 1].plot(ticks, [t.stress for t in telemetry], label='Stress', color='#d62728')
    axs[0, 1].plot(ticks, [t.fear for t in telemetry], label='Fear', color='#ff7f0e')
    axs[0, 1].plot(ticks, [t.drive for t in telemetry], label='Drive (Hunger)', color='#9467bd')
    axs[0, 1].set_title('Emotional / Motivational State', fontweight='bold')
    axs[0, 1].legend()
    axs[0, 1].grid(True, alpha=0.3)

    # 3. Motor Output
    axs[1, 0].plot(ticks, [t.thrust for t in telemetry], label='Thrust', color='#17becf')
    ...
    # 4. Trajectory
    axs[1, 1].plot([t.pos_x for t in telemetry], [t.pos_y for t in telemetry], 
    ...
    # 6. Memory Growth
    axs[2, 1].plot(ticks, [t.landmark_count for t in telemetry], label='Landmarks', color='#e377c2')
    axs[2, 1].plot(ticks, [t.grid_cells for t in telemetry], label='Grid Cells', color='#7f7f7f')
...
def print_summary(telemetry, replay):
    ...
    if replay:
        ...
        print(f"Final Action         : Thrust={mo.get('thrust',0):.2f} | Steer={mo.get('steer',0):.2f}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Project BIOS Log Analyzer & Visualizer")
    parser.add_argument("--telemetry", default="telemetry.jsonl", help="Telemetry file path")
    parser.add_argument("--replay", default="replay.jsonl", help="Replay file path")
    parser.add_argument("--save", type=str, help="Save plot to this image file (e.g. run1_analysis.png)")
    parser.add_argument("--summary", action="store_true", help="Show only summary, no plot")
