...
def _lerp_color(base: Tuple[int,int,int], t: float, dark=(15,17,21)) -> Tuple[int,int,int]:
...
def _alpha_surf(color: Tuple[int,int,int], alpha: int, size: Tuple[int,int]) -> pygame.Surface:
...
class _Visualizer:
    ...
    def _draw_cells(self, surf: pygame.Surface) -> None:
        ...
        for (cx, cy), cell in self.memory._grid.items():
            ...
            # Quick cull
            if sx + cell_px < 0 or sx > self.WIDTH:  continue
            if sy + cell_px < 0 or sy > self.HEIGHT: continue
    ...
    def _draw_path(self, surf: pygame.Surface) -> None:
        ...
        pygame.draw.lines(surf, (*PATH_COLOR, 160), False, pts, max(1, int(self._scale * 0.1 + 0.5)))
    ...
    def _draw_hud(self, surf: pygame.Surface, font: pygame.font.Font, small_font: pygame.font.Font) -> None:
        mem = self.memory
        ...
        for color, label, active in legend:
            ...
            surf.blit(small_font.render(label, True, TEXT_PRIMARY if active else TEXT_SECONDARY), (lx + 18, ly - 1))
            ly += 20

        # Controls hint
        hints = ["scroll: zoom", "drag: pan", "H/F/L/P: layers", "R: fit", "S: screenshot", "Q: quit"]
        hx = self.WIDTH - 160
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    self._scale = max(0.2, self._scale * (1.15 if event.y > 0 else 1 / 1.15))
            ...
            if hcell:
                tip_lines += [f"hazard {hcell.hazard:.3f}", f"food   {hcell.food:.3f}", f"tick   {hcell.last_updated_tick}"]
            ...
            surf.blit(tip, (min(mx + 14, self.WIDTH - tw - 4), min(my - 10, self.HEIGHT - th - 4)))
...
if __name__ == "__main__":
    ...
    for t in range(600):
        ...
        if (40 < pos.x < 120 and -20 < pos.y < 60) or pos.x < -50:
            v = rng.random() * 0.8
            if key not in mem._grid: mem._grid[key] = GridCell()
            mem._grid[key].hazard = min(1.0, mem._grid[key].hazard + v * (1 - mem._grid[key].hazard))
            mem._grid[key].last_updated_tick = t
        if (-30 < pos.x < 30 and 40 < pos.y < 120) or pos.x > 100:
            v = rng.random() * 0.7
            if key not in mem._grid: mem._grid[key] = GridCell()
            mem._grid[key].food = min(1.0, mem._grid[key].food + v * (1 - mem._grid[key].food))
            mem._grid[key].last_updated_tick = t
        if rng.random() > 0.97:
            lid = rng.randint(0, 4)
            mem._landmarks[lid] = LandmarkRecord(
                pos=Vector2(pos.x + rng.uniform(-8, 8), pos.y + rng.uniform(-8, 8)),
                last_seen_tick=t, observation_count=rng.randint(1, 20)
            )
