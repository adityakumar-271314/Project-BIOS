...
class Renderer:
    ...
    def render_path(self, session, current_frame: int, camera, ui):
        if current_frame <= 0:
            return
        trail_points = [
            camera.world_to_screen(t.pos_x, t.pos_y)
            for f_idx in range(current_frame + 1)
            if (t := session.get_tick(f_idx))
        ]
        if len(trail_points) > 1:
            pygame.draw.lines(
                self.screen, GRAY, False, trail_points, ui.w(2) if ui else 2
            )
        ...
        if agent_surf:
            ...
            if ui:
                ...
                agent_surf = pygame.transform.scale(
                    agent_surf, (target_size, target_size)
                )
            ...
            pygame.draw.circle(
                self.screen, AGENT_COLOR, screen_pos, ui.r(20) if ui else 20
            )

    def render_raw_telemetry_path(self, session, camera, ui):
        raw_points = [
            camera.world_to_screen(pos[0], pos[1]) for pos in session.raw_path
        ]
        if len(raw_points) > 1:
            pygame.draw.lines(
                self.screen, ORANGE, False, raw_points, ui.w(1) if ui else 1
            )
