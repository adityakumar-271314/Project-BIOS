...
class GraphsOverlay:
    ...
    def render(self, screen, session, playback, camera, ui=None):
        ...
        pygame.draw.rect(
            screen, (8, 9, 12), (pad_x, y_top + 2, box_w, box_h), border_radius=radius
        )
        # Layer 2: Core graph canvas frame
        pygame.draw.rect(
            screen, (18, 19, 26), (pad_x, y_top, box_w, box_h), border_radius=radius
        )
        # Layer 3: Thickened structural border accent
        pygame.draw.rect(
            screen,
            (35, 38, 50),
            (pad_x, y_top, box_w, box_h),
            width=border_thickness_1,
        ...
        pygame.draw.rect(
            screen,
            (55, 60, 78),
            (pad_x + 2, y_top + 2, box_w - 4, box_h - 4),
            width=border_thickness_2,
            border_radius=radius,
        )
        ...
        pygame.draw.line(
            graph_surface,
            (65, 70, 95),
            (axis_left_margin, legend_deck_height),
            (axis_left_margin, y_zero - y_top),
            ui.w(2) if ui else 2,
        )
        ...
        for val in [0.0, 0.5, 1.0]:
            ...
            pygame.draw.line(
                graph_surface,
                (28, 30, 42),
                (axis_left_margin, internal_y),
                (box_w - axis_right_margin, internal_y),
                ui.w(1) if ui else 1,
            )
            ...
            graph_surface.blit(
                lbl,
                (
                    axis_left_margin - (ui.x(32) if ui else 32),
                    internal_y - (ui.y(6) if ui else 6),
                ),
            )
        ...
        x_step = (
            graph_canvas_w / (playback.total_frames - 1)
        ...
        for name, values in metrics.items():
            ...
            poly_points = (
                [(points[0][0], y_zero - y_top)]
                + points
                + [(points[-1][0], y_zero - y_top)]
            )
        ...
        for name, values in metrics.items():
            ...
            if len(points) == 1:
                pygame.draw.circle(
                    graph_surface, color, points[0], ui.r(3) if ui else 3
                )
            elif len(points) > 1:
                # Draw the historical line track
                pygame.draw.lines(
                    graph_surface, color, False, points, ui.w(2) if ui else 2
                )
                ...
                pygame.draw.circle(
                    graph_surface, color, leading_point, ui.r(4) if ui else 4
                )
                # Inner dark core to make it look sharp and modern against the background
                pygame.draw.circle(
                    graph_surface, (18, 19, 26), leading_point, ui.r(1.5) if ui else 1.5
                )
        ...
        for name, (peak_frame, peak_val) in peaks.items():
            if peak_frame != -1 and peak_val > 0.4:
                ...
                py = int(
                    (y_zero - y_top) - (max(0.0, min(peak_val, 1.0)) * graph_canvas_h)
                )
                ...
                if px <= int(axis_left_margin + (curr_frame * x_step)):
                    ...
                    pygame.draw.circle(
                        graph_surface, (18, 19, 26), (px, py), ui.r(2) if ui else 2
                    )
        ...
        for idx, (name, color) in enumerate(self.colors.items()):
            ...
            pygame.draw.rect(
                screen,
                color,
                (x_pos, block_y, block_size, block_size),
                border_radius=ui.r(1) if ui else 1,
            )

            lbl = self.font.render(
                f" {name.lower()}: {curr_val:.2f}", True, (160, 175, 190)
            )
