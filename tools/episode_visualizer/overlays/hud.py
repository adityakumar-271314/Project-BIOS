...
class HUDOverlay:
    ...
    def _render_sidebar(self, screen, tick, screen_w, ui):
        ...
        pygame.draw.rect(
            screen, PANEL_BG, (sb_x, sb_y, sb_w, total_h), 0, ui.r(6) if ui else 6
        )
        pygame.draw.rect(
            screen,
            PANEL_BORDER,
            (sb_x, sb_y, sb_w, total_h),
            ui.w(2) if ui else 2,
            ui.r(6) if ui else 6,
        )
        ...
        conf_color = (
            GREEN
            if tick.confidence >= 0.9
            else (ORANGE if tick.confidence >= 0.6 else RED)
        )
        self._render_bar(
            screen,
            sb_x + pad,
        ...
        curr_y = self._render_labeled_bar(
            screen, "FEAR", tick.fear, RED, sb_x + pad, curr_y, inner_w, row_h, ui
        )
        curr_y = self._render_labeled_bar(
            screen,
            "STRESS",
        ...
        curr_y = self._render_labeled_bar(
            screen, "DRIVE", tick.drive, BLUE, sb_x + pad, curr_y, inner_w, row_h, ui
        )
        curr_y = self._render_labeled_bar(
            screen, "ENERGY", tick.energy, GREEN, sb_x + pad, curr_y, inner_w, row_h, ui
        )
        self._render_labeled_bar(
            screen,
            "INTEGRITY",
            tick.integrity,
            YELLOW,
    ...
    def _render_bar(self, screen, x, y, w, h, fill_ratio, color, ui):
        ...
        if norm_ratio > 0:
            pygame.draw.rect(
                screen, color, (x, y, int(w * norm_ratio), h), 0, ui.r(2) if ui else 2
            )
