...
class TimelineOverlay:
    ...
    def render(self, screen, session, playback, camera, ui=None):
        ...
        pygame.draw.line(screen, DARK_GRAY, (0, y_top), (screen_w, y_top), ui.w(2) if ui else 2)
        
        progress = playback.current_frame / (playback.total_frames - 1) if playback.total_frames > 1 else 0
        scrub_x = int(progress * screen_w)
        
        track_y = y_top + (timeline_h // 2) - ui.h(2) if ui else y_top + (timeline_h // 2) - 2
        track_h = ui.h(4) if ui else 4
        
        pygame.draw.rect(screen, RED, (0, track_y, scrub_x, track_h))
        pygame.draw.rect(screen, DARK_GRAY, (scrub_x, track_y, screen_w - scrub_x, track_h))
        
        knob_color = WHITE if not self.dragging else RED
        pygame.draw.circle(screen, knob_color, (scrub_x, y_top + (timeline_h // 2)), ui.r(8) if ui else 8)
