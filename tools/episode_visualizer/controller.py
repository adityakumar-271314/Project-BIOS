import pygame
from tools.episode_visualizer.config import WINDOW_WIDTH, WINDOW_HEIGHT, FPS, BLACK, WHITE, GREEN, RED
...
class VisualizerController:
    def __init__(self):
        ...
        self.app_state = "BROWSER"  # Modes: "BROWSER", "PLAYBACK", "ERROR"
        self.error_message = ""
        
        # Browser selection indexes
        ...
        self.overlays = [HUDOverlay(), self.timeline_overlay, GraphsOverlay()]
        self.running = True
    ...
    def refresh_browser_list(self):
        try:
            ...
            if not self.episodes_list:
                self.trigger_error("Empty Episode Repository Encountered.")
    ...
    def run(self):
        while self.running:
            ...
            # Update loops
            if self.app_state == "PLAYBACK" and self.playback:
    ...
    def handle_browser_events(self, event):
        ...
        if event.key == pygame.K_UP:
            ...
            try:
                ...
                self.trigger_error(f"Live Reconstruction Error: {str(e)}")

    def render_browser(self):
        self.screen.fill(BLACK)
        title = self.font.render("SYSTEM EPISODE BROWSER (UP/DOWN to navigate, ENTER to select)", True, WHITE)
    ...
    def render_error_screen(self):
        ...
        esc_msg = self.font.render("Press ESC to return to Episode Browser", True, WHITE)
