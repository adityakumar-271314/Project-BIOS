...
class MetadataPanel:
    ...
    def render(
        ...
        meta_title = self.scene.controller.font.render(
            "METADATA PREVIEW", True, ACCENT_CYAN
        )
        ...
        if state.selected_metadata:
            ...
            for key, val in state.selected_metadata.items():
                ...
                k_render = self.scene.controller.small_font.render(
            ...
            no_meta_str = (
                ...
                if self.scene.filtered_list
                else "No Selection Focus Found"
            )
            no_meta = self.scene.controller.small_font.render(
                no_meta_str, True, TEXT_MUTED
            )
            screen.blit(
                no_meta,
                no_meta.get_rect(
