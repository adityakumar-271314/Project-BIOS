...
class FilterPanel:
    ...
    def handle_mouse_down(self, pos) -> bool:
        ...
        for i, et in enumerate(state.event_types):
            if self.get_chip_rect(i).collidepoint(pos):
                state.active_event_filter = None if state.active_event_filter == et else et
                self.scene.apply_filters()
                return True
    ...
    def update_slider(self, pos):
        ...
        if state.dragging_slider == "sig":
            ...
            if state.dragging_slider == "tick_start":
                state.tick_start = max(state.min_repository_ticks, min(val, state.tick_end - 1))
            else:
                state.tick_end = max(state.tick_start + 1, min(val, state.max_repository_ticks))
        self.scene.apply_filters()
    ...
    def render(self, screen: pygame.Surface):
        ...
        pygame.draw.rect(screen, PANEL_BORDER, panel, width=max(1, ui.r(1)), border_radius=ui.r(8))
        ...
        for i, et in enumerate(state.event_types):
            ...
            if is_active:
                pygame.draw.rect(screen, ACCENT_CYAN, rect, width=max(1, ui.r(1)), border_radius=ui.r(6))

            txt = self.scene.controller.small_font.render(et.replace("_", " ").upper(), True, color)
            screen.blit(txt, (rect.x + ui.w(10), rect.y + (rect.height // 2 - txt.get_height() // 2)))
        ...
        if fill_w > 0:
            pygame.draw.rect(
                screen, ACCENT_CYAN, (slider_rect.x, slider_rect.y, fill_w, slider_rect.height), border_radius=ui.r(4)
            )
        ...
        start_x = tick_rect.x + int(tick_rect.width * ((state.tick_start - state.min_repository_ticks) / denom))
        end_x = tick_rect.x + int(tick_rect.width * ((state.tick_end - state.min_repository_ticks) / denom))

        if end_x > start_x:
            pygame.draw.rect(
                screen, ACCENT_BLUE, (start_x, tick_rect.y, end_x - start_x, tick_rect.height), border_radius=ui.r(4)
            )

        pygame.draw.circle(screen, TEXT_WHITE, (start_x, tick_rect.y + tick_rect.height // 2), ui.r(6))
        pygame.draw.circle(screen, TEXT_WHITE, (end_x, tick_rect.y + tick_rect.height // 2), ui.r(6))
        ...
        reset_txt = self.scene.controller.small_font.render("RESET ALL FILTERS", True, TEXT_WHITE)
