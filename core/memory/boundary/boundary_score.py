...
class BoundaryScore:
    ...
    def _score_start(
        ...
        continuous_score = (importance * 0.4) + (attention * 0.3) + (surprise_norm * 0.3)
        ...
        if triggers:
            raw_score = (rule_score * self.rule_weight) + (continuous_score * self.metric_weight)
        else:
    ...
    def _score_end(
        ...
        attention_drop = 0.0
        if prev_frame:
            attention_drop = max(0.0, prev_frame.attention_score - curr_frame.attention_score)

        low_importance_bonus = max(0.0, 0.5 - curr_frame.importance) if prev_frame and prev_frame.importance > 0.5 else 0.0
