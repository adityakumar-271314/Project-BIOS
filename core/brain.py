...
class GoalStackManager:
    ...
    def evaluate_priorities(self, ehe, sensor_data: SensorPacket, spatial_bias: Vector2) -> MotorOutput:
        food_force = 0.0
        hazard_force = 0.0
        wall_force = 0.0
        ...
        for obj in sensed:
            ...
            if obj.type == "food":
                ...
                food_force -= obj.angle * (ehe.drive * dist_mult * self.cfg.food_force_multiplier)

            # PUSH AWAY FROM FIRE
            if obj.type == "hazard":
                # Massive repulsion scaled by fear
                hazard_force += obj.angle * (ehe.fear * self.cfg.hazard_force_multiplier)
        ...
        if rl < self.cfg.wall_threshold:
            wall_force += (self.cfg.wall_threshold - rl) * self.cfg.wall_force_multiplier
        if rr < self.cfg.wall_threshold:
            wall_force -= (self.cfg.wall_threshold - rr) * self.cfg.wall_force_multiplier
        ...
        final_steer = (total_steer * self.cfg.steer_smoothing_current) + (self.last_steer * self.cfg.steer_smoothing_previous)
        self.last_steer = final_steer

        # THRUST CALCULATION
        thrust = self.cfg.thrust_base + (ehe.fear * self.cfg.thrust_fear_coeff) - (ehe.stress * self.cfg.thrust_stress_coeff)
    ...
    def drift_logic(self):
        ...
        if self.drift_timer <= 0:
            self.drift_timer = self.rng.randint(self.cfg.drift_interval_min, self.cfg.drift_interval_max)
            self.drift_angle = self.rng.uniform(-self.cfg.drift_angle_max, self.cfg.drift_angle_max)
